...
// devuelve una referencia, devuelve signal no una copia.
high_temp_sig_t& get_high_temp_signal(){
	static high_temp_sig_t sig; 
	// conserva su valor entre llamadas
	// se crean la primera vez que invocas a al funcion.
	return sig;
}

low_temp_sig_t& get_low_temp_signal(){
	static low_temp_sig_t sig; 		// se mantiene viva entre señales
	return sig;
}
...
Grid createGrid(  	uint32_t rows, uint32_t cols, 
			float low_threshold, float high_threshold, 
			uint32_t max_idle,
			float default_temp){
	Grid grid;
	
	if(low_threshold >= high_threshold || low_threshold < -100 
	|| low_threshold > 100 || high_threshold < -100 || high_threshold > 100
	|| default_temp < -100 || default_temp > 100 || max_idle > 50
	|| rows == 0 || cols == 0){
		grid = empty_grid;
	}
	else{
		...
		grid.sensors = new Sensor * [rows]; // return the direction of the first sensor Sensor **
		for(uint32_t i = 0; i < rows; i++){
			grid.sensors[i] = new Sensor [cols]; // return the begining of the row Sensor *
...
void freeGrid(Grid& grid){
	if(grid.sensors){
		// libero cada una de las filas,
		for(uint32_t i = 0; i < grid.rows; i++){
			delete [] grid.sensors[i];
		}
		// libero el vector de direcciones de filas
		delete [] grid.sensors;
		grid = empty_grid;
	}
}
...
bool aux_check(const Position &p, uint32_t rows, uint32_t cols){
...
bool swapSensors(Grid& grid, const Position& a, const Position& b){
	...
	if(aux_check(a, grid.rows, grid.cols) && aux_check(b, grid.rows, grid.cols)){
		swaped = true;
